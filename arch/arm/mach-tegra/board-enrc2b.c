/*
 * arch/arm/mach-tegra/board-enrc2b.c
 *
...
//flashlight
static void config_enrc2b_flashlight_gpios(void)
{
	int ret;
...
static struct TPS61310_flashlight_platform_data enrc2b_flashlight_data = {
	.gpio_init  = config_enrc2b_flashlight_gpios,
...
static struct i2c_board_info i2c_flashlight_devices[] = {
	{
		...
		.platform_data = &enrc2b_flashlight_data,
		.irq = -1,
	},
};
...
static void tegra_vibrator_init(void)
{
	if(board_get_sku_tag() == 0x34600) {
		if( htc_get_pcbid_info() == PROJECT_PHASE_XA ){
			...
			vibrator_data.ena_gpio = TEGRA_GPIO_PH0;
		}
		else{
			vibrator_data.pwm_gpio = -1;
...
#ifdef CONFIG_BT
static struct platform_device enrc2b_rfkill = {
	.name = "enrc2b_rfkill",
	.id = -1,
};
#endif

static __initdata struct tegra_clk_init_table enrc2b_clk_init_table[] = {
...
static struct tegra_i2c_platform_data enrc2b_i2c1_platform_data = {
	.adapter_nr	= 0,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data enrc2b_i2c2_platform_data = {
	.adapter_nr	= 1,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data enrc2b_i2c3_platform_data = {
	.adapter_nr	= 2,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data enrc2b_i2c4_platform_data = {
	.adapter_nr	= 3,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data enrc2b_i2c5_platform_data = {
	.adapter_nr	= 4,
	.bus_count	= 1,
	.bus_clk_rate	= { 100000, 0 },
...
static struct htc_asoc_platform_data enrc2b_audio_pdata;

static struct platform_device enrc2b_audio_device = {
	.name	= "tegra-snd-aic3008",
	.id	= 0,
	.dev	= {
		.platform_data  = &enrc2b_audio_pdata,
	},
};

#if (defined(CONFIG_SND_AMP_TFA9887))
static struct tfa9887_platform_data enrc2b_speaker_amp_data = {
...
static struct i2c_board_info i2c_speaker_amp_devices[] = {
	{
		...
		.platform_data = &enrc2b_speaker_amp_data,
		.irq = -1,
	},
};
#endif
...
static struct spi_board_info enrc2b_spi_board_info_rawchip[] __initdata = {
       {
...
EXPORT_SYMBOL_GPL(enrc2b_spi_board_info_rawchip);
...
static void enrc2b_i2c_init(void)
{
	tegra_i2c_device1.dev.platform_data = &enrc2b_i2c1_platform_data;
	tegra_i2c_device2.dev.platform_data = &enrc2b_i2c2_platform_data;
	tegra_i2c_device3.dev.platform_data = &enrc2b_i2c3_platform_data;
	tegra_i2c_device4.dev.platform_data = &enrc2b_i2c4_platform_data;
	tegra_i2c_device5.dev.platform_data = &enrc2b_i2c5_platform_data;
...
static void __init uart_debug_init(void)
{
	...
	enrc2b_uart_devices[0] = &debug_uarta_device;
...
static void __init enrc2b_uart_init(void)
{
	int i;
	struct clk *c;
	...
	enrc2b_uart_pdata.parent_clk_list = uart_parent_clk;
	enrc2b_uart_pdata.parent_clk_count = ARRAY_SIZE(uart_parent_clk);

/* GPS use UARTE
	enrc2b_loopback_uart_pdata.parent_clk_list = uart_parent_clk;
	enrc2b_loopback_uart_pdata.parent_clk_count =
						ARRAY_SIZE(uart_parent_clk);
	enrc2b_loopback_uart_pdata.is_loopback = true;
*/

	tegra_uarta_device.dev.platform_data = &enrc2b_uart_pdata;
	tegra_uartb_device.dev.platform_data = &enrc2b_uart_pdata;
	tegra_uartc_device.dev.platform_data = &enrc2b_uart_pdata;
	tegra_uartd_device.dev.platform_data = &enrc2b_uart_pdata;
	tegra_uarte_device.dev.platform_data = &enrc2b_uart_pdata; // Used for BRCM GPS.
...
/* GPS uses UARTE
	...
	tegra_uarte_device.dev.platform_data = &enrc2b_loopback_uart_pdata;
*/
	...
	platform_add_devices(enrc2b_uart_devices,
				ARRAY_SIZE(enrc2b_uart_devices));
}
...
static struct tegra_spi_platform_data enrc2b_spi_pdata = {
	.is_dma_based		= true,
...
static void __init enrc2b_spi_init(void)
{
	int i;
	struct clk *c;
	...
	enrc2b_spi_pdata.parent_clk_list = spi_parent_clk;
	enrc2b_spi_pdata.parent_clk_count = ARRAY_SIZE(spi_parent_clk);

	spi_register_board_info(spi_board_info_audio, ARRAY_SIZE(spi_board_info_audio));
	spi_register_board_info(enrc2b_spi_board_info_rawchip, ARRAY_SIZE(enrc2b_spi_board_info_rawchip));
	platform_device_register(&tegra_spi_device2);
	tegra_spi_device4.dev.platform_data = &enrc2b_spi_pdata;
...
static struct platform_device *enrc2b_devices[] __initdata = {
	&tegra_pmu_device,
	&tegra_rtc_device,
	&tegra_udc_device,
...
static struct synaptics_i2c_rmi_platform_data edge_ts_3k_data_XB[] = {
	...
	},
	{
		...
		.psensor_detection = 1,
		.config = {
			,
			...
			0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
			0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x80,
			0x80,0x80,0x80,0x80,0x80,0x80,0x80,0x00,0x0F,0x00
		}
	},
	{
		.version = 0x3332, /* fw32 */
...
static int __init enrc2b_touch_init(void)
{
	int retval = 0;
	struct board_info BoardInfo;
	pr_info("[TP]Enter enrc2b_touch_init\n");
...
static struct tegra_usb_phy_platform_ops hsic_xmm_plat_ops = {
	.post_suspend = enrc2b_usb_hsic_postsupend,
	.pre_resume = enrc2b_usb_hsic_preresume,
	.port_power = enrc2b_usb_hsic_phy_ready,
	.post_phy_off = enrc2b_usb_hsic_phy_off,
};
...
static void enrc2b_usb_init(void)
{
...
static void config_tegra_usb_reset_wdt_gpios()
{
	...
	pr_info("[CABLE] %s:\n", __func__);
	/*
	** TEGRA_GPIO_PP4:RESET_EN_CLR L:clear WDT (for XA)
	...
	*/
	...
	if (htc_get_pcbid_info() >= PROJECT_PHASE_XA) {
		...
		ret = gpio_direction_output(TEGRA_GPIO_PS6,0);
		if (ret < 0) {
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.reset_en_clr_gpio	= TEGRA_GPIO_PZ4,
	.usb_dpdn_switch	= tegra_usb_dpdn_switch,
	.enable_reset_wdt			= false,
	/*.mpp_data = {
...
static void enrc2b_cable_detect_init(void)
{
	platform_device_register(&cable_detect_device);
}


static struct platform_device *enrc2b_audio_devices[] __initdata = {
	&tegra_ahub_device,
	&tegra_dam_device0,
	&tegra_dam_device1,
	&tegra_dam_device2,
...
static void enrc2b_audio_init(void)
{
	enrc2b_audio_codec_init(&enrc2b_audio_pdata);

	platform_add_devices(enrc2b_audio_devices,
			ARRAY_SIZE(enrc2b_audio_devices));
}

static void enrc2b_gps_init(void)
{
	int rc;
...
static __initdata struct tegra_pingroup_config enrc2b_pinmux_modem[] = {
    ...
    DEFAULT_PINMUX(KB_ROW13,        RSVD,            NORMAL,    NORMAL,     INPUT),     //MDM_BB2AP_RST2
    DEFAULT_PINMUX(LCD_PWR2,        DISPLAYA,        NORMAL,    NORMAL,     OUTPUT),    //MDM_AP2BB_SLAVE_WAKEUP_1
    DEFAULT_PINMUX(KB_ROW10,        RSVD,            NORMAL,    NORMAL,     INPUT),     //MDM_BB2AP_HOST_WAKEUP
...
#if 0
static __initdata struct tegra_pingroup_config enrc2b_pinmux_modem_XB[] = {
    DEFAULT_PINMUX(KB_ROW13,        RSVD,            PULL_DOWN, NORMAL,     INPUT),     //MDM_BB2AP_RST2
};
#endif

static void enrc2b_baseband_init(void)
{
#if 0
	int board_id = htc_get_pcbid_info();
	int sku_id = board_get_sku_tag();
	const int Global_SKU = 0x00034600;
#endif
	...
	{
		tegra_pinmux_config_table(enrc2b_pinmux_modem_XB, ARRAY_SIZE(enrc2b_pinmux_modem_XB));
	}
#endif
...
static struct gpio_keys_platform_data ENRC2_PROJECT_keys_platform_data = {
	.buttons	= ENRC2_PROJECT_keys,
	.nbuttons	= ARRAY_SIZE(ENRC2_PROJECT_keys),
...
static struct platform_device ENRC2_PROJECT_keys_device = {
	.name   = "gpio-keys",
	.id     = 0,
	.dev    = {
		.platform_data  = &ENRC2_PROJECT_keys_platform_data,
	},
};

static int __init ENRC2_PROJECT_keys_init(void)
{
	int i;
	pr_info("[KEY]Enter ENRC2_PROJECT_keys_init\n");
	for (i = 0; i < ARRAY_SIZE(ENRC2_PROJECT_keys); i++)
		tegra_gpio_enable(ENRC2_PROJECT_keys[i].gpio);

	platform_device_register(&ENRC2_PROJECT_keys_device);
...
static void __init enrc2b_init(void)
{
	...
	tegra_clk_init_from_table(enrc2b_clk_init_table);
	enrc2b_pinmux_init();
	enrc2b_i2c_init();
	enrc2b_spi_init();
	enrc2b_uart_init();
	...
	platform_add_devices(enrc2b_devices, ARRAY_SIZE(enrc2b_devices));
	if (!is_tegra_debug_uartport_hs()) {
	...
	enrc2b_regulator_init();
	ENRC2_PROJECT_keys_init();
	enrc2b_sdhci_init();
#ifdef CONFIG_TEGRA_EDP_LIMITS
	enrc2b_edp_init();
#endif
...
MACHINE_START(ENRC2B, "enrc2b")
	.boot_params    = 0x80000100,
	.map_io         = tegra_map_common_io,
	.reserve        = enrc2b_reserve,
	.init_early	= tegra_init_early,
	.init_irq       = tegra_init_irq,
	.timer          = &tegra_timer,
	.init_machine   = enrc2b_init,
MACHINE_END
