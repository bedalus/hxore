/*
 * arch/arm/mach-tegra/board-enrc2b-panel.c
 *
...
#define enrc2b_lvds_shutdown	TEGRA_GPIO_PL2
#define enrc2b_hdmi_hpd		TEGRA_GPIO_PN7

#define enrc2b_dsi_panel_reset	TEGRA_GPIO_PW0

#define enrc2b_lcd_2d_3d		TEGRA_GPIO_PH1
#define ENRC2B_STEREO_3D		0
#define ENRC2B_STEREO_2D		1

#define enrc2b_lcd_swp_pl		TEGRA_GPIO_PH2
#define ENRC2B_STEREO_LANDSCAPE	0
#define ENRC2B_STEREO_PORTRAIT	1
...
static int enrc2b_disp1_check_fb(struct device *dev, struct fb_info *info);
 ...
 */
static struct platform_tegra_pwm_backlight_data enrc2b_disp1_backlight_data = {
	.which_dc		= 0,
...
static struct platform_device enrc2b_disp1_backlight_device = {
	...
	.id	= -1,
	.dev	= {
		.platform_data = &enrc2b_disp1_backlight_data,
	},
};

#ifdef CONFIG_TEGRA_DC
static int enrc2b_hdmi_vddio_enable(void)
{
	return 0;
}

static int enrc2b_hdmi_vddio_disable(void)
{
	return 0;
}

static int enrc2b_hdmi_enable(void)
{
	REGULATOR_GET(enrc2b_hdmi_reg, "avdd_hdmi");
	regulator_enable(enrc2b_hdmi_reg);

	REGULATOR_GET(enrc2b_hdmi_pll, "avdd_hdmi_pll");
	regulator_enable(enrc2b_hdmi_pll);

failed:
	return 0;
}

static int enrc2b_hdmi_disable(void)
{

	regulator_disable(enrc2b_hdmi_reg);
	regulator_put(enrc2b_hdmi_reg);
	enrc2b_hdmi_reg = NULL;

	regulator_disable(enrc2b_hdmi_pll);
	regulator_put(enrc2b_hdmi_pll);
	enrc2b_hdmi_pll = NULL;

	return 0;
}
static struct resource enrc2b_disp1_resources[] = {
	{
		.name	= "irq",
	...
	{
		.name	= "fbmem",
		.start	= 0,	/* Filled in by enrc2b_panel_init() */
		.end	= 0,	/* Filled in by enrc2b_panel_init() */
		.flags	= IORESOURCE_MEM,
	},
	{
		.name	= "dsi_regs",
...
static struct resource enrc2b_disp2_resources[] = {
	{
		.name	= "irq",
...
static struct tegra_fb_data enrc2b_hdmi_fb_data = {
	.win		= 0,
	.xres		= 1280,
	.yres		= 720,
	.bits_per_pixel	= 32,
...
static struct tegra_dc_out enrc2b_disp2_out = {
	.type		= TEGRA_DC_OUT_HDMI,
	.flags		= TEGRA_DC_OUT_HOTPLUG_HIGH,
	.parent_clk	= "pll_d2_out0",

	.dcc_bus	= 3,
	.hotplug_gpio	= enrc2b_hdmi_hpd,
...
static int bridge_reset(void)
{
	...
	if(g_display_on)
	{
		REGULATOR_GET(enrc2b_dsi_reg, "avdd_dsi_csi");
		regulator_enable(enrc2b_dsi_reg);
...
static int enrc2b_dsi_panel_disable(void)
{
	int err = 0;
	...
	REGULATOR_GET(enrc2b_dsi_reg, "avdd_dsi_csi");
	regulator_disable(enrc2b_dsi_reg);
...
static void enrc2b_stereo_set_mode(int mode)
{
	switch (mode) {
	case TEGRA_DC_STEREO_MODE_2D:
		/*gpio_set_value(TEGRA_GPIO_PH1, ENRC2B_STEREO_2D);*/
		break;
	case TEGRA_DC_STEREO_MODE_3D:
		/*gpio_set_value(TEGRA_GPIO_PH1, ENRC2B_STEREO_3D);*/
		break;
	}
}

static void enrc2b_stereo_set_orientation(int mode)
{
	switch (mode) {
	case TEGRA_DC_STEREO_LANDSCAPE:
		/*gpio_set_value(TEGRA_GPIO_PH2, ENRC2B_STEREO_LANDSCAPE);*/
		break;
	case TEGRA_DC_STEREO_PORTRAIT:
		/*gpio_set_value(TEGRA_GPIO_PH2, ENRC2B_STEREO_PORTRAIT);*/
		break;
	}
}

#ifdef CONFIG_TEGRA_DC
static int enrc2b_dsi_panel_postsuspend(void)
{
	int err = 0;
...
struct tegra_dsi_out enrc2b_dsi = {
	.n_data_lanes = 2,
...
static struct tegra_stereo_out enrc2b_stereo = {
	.set_mode		= &enrc2b_stereo_set_mode,
	.set_orientation	= &enrc2b_stereo_set_orientation,
};
...
static struct tegra_fb_data enrc2b_dsi_fb_data = {
	.win		= 0,
	.xres		= 720,
	.yres		= 1280,
	.bits_per_pixel	= 32,
...
static struct tegra_dc_out enrc2b_disp1_out = {
	.align		= TEGRA_DC_ALIGN_MSB,
	.order		= TEGRA_DC_ORDER_RED_BLUE,
	...
	.modes		= enrc2b_dsi_modes,
	.n_modes	= ARRAY_SIZE(enrc2b_dsi_modes),

	.dsi		= &enrc2b_dsi,
	.stereo		= &enrc2b_stereo,

	.enable		= enrc2b_dsi_panel_enable,
	.disable	= enrc2b_dsi_panel_disable,
	.postsuspend	= enrc2b_dsi_panel_postsuspend,
...
static struct tegra_dc_platform_data enrc2b_disp1_pdata = {
	.flags		= TEGRA_DC_FLAG_ENABLED,
	.default_out	= &enrc2b_disp1_out,
	.emc_clk_rate	= 204000000,
	.fb		= &enrc2b_dsi_fb_data,
};

static struct nvhost_device enrc2b_disp1_device = {
	.name		= "tegradc",
	.id		= 0,
	.resource	= enrc2b_disp1_resources,
	.num_resources	= ARRAY_SIZE(enrc2b_disp1_resources),
	.dev = {
		.platform_data = &enrc2b_disp1_pdata,
	},
};

static int enrc2b_disp1_check_fb(struct device *dev, struct fb_info *info)
{
	return info->device == &enrc2b_disp1_device.dev;
}

static struct nvhost_device enrc2b_disp2_device = {
	.name		= "tegradc",
	.id		= 1,
	.resource	= enrc2b_disp2_resources,
	.num_resources	= ARRAY_SIZE(enrc2b_disp2_resources),
	.dev = {
		.platform_data = &enrc2b_disp2_pdata,
	},
};
#endif

#if defined(CONFIG_TEGRA_NVMAP)
static struct nvmap_platform_carveout enrc2b_carveouts[] = {
	[0] = NVMAP_HEAP_CARVEOUT_IRAM_INIT,
	[1] = {
		.name		= "generic-0",
		.usage_mask	= NVMAP_HEAP_CARVEOUT_GENERIC,
		.base		= 0,	/* Filled in by enrc2b_panel_init() */
		.size		= 0,	/* Filled in by enrc2b_panel_init() */
		.buddy_size	= SZ_32K,
	},
};

static struct nvmap_platform_data enrc2b_nvmap_data = {
	.carveouts	= enrc2b_carveouts,
	.nr_carveouts	= ARRAY_SIZE(enrc2b_carveouts),
};

static struct platform_device enrc2b_nvmap_device = {
	.name	= "tegra-nvmap",
	.id	= -1,
	.dev	= {
		.platform_data = &enrc2b_nvmap_data,
	},
};
#endif

static struct platform_device *enrc2b_gfx_devices[] __initdata = {
#if defined(CONFIG_TEGRA_NVMAP)
	&enrc2b_nvmap_device,
#endif
	&tegra_pwfm0_device,
...
static struct platform_device *enrc2b_bl_devices[]  = {
	&enrc2b_disp1_backlight_device,
};
 ...
 */
struct early_suspend enrc2b_panel_early_suspender;
...
static void enrc2b_panel_late_resume(struct early_suspend *h)
{
	unsigned i;
...
int __init enrc2b_panel_init(void)
{
	int err;
	int i = 0;
...
#if defined(CONFIG_TEGRA_NVMAP)
	enrc2b_carveouts[1].base = tegra_carveout_start;
	enrc2b_carveouts[1].size = tegra_carveout_size;
#endif

	tegra_gpio_enable(enrc2b_hdmi_hpd);
	gpio_request(enrc2b_hdmi_hpd, "hdmi_hpd");
	gpio_direction_input(enrc2b_hdmi_hpd);

	tegra_gpio_enable(enrc2b_lcd_2d_3d);
	gpio_request(enrc2b_lcd_2d_3d, "lcd_2d_3d");
	gpio_direction_output(enrc2b_lcd_2d_3d, 0);
	enrc2b_stereo_set_mode(enrc2b_stereo.mode_2d_3d);

	tegra_gpio_enable(enrc2b_lcd_swp_pl);
	gpio_request(enrc2b_lcd_swp_pl, "lcd_swp_pl");
	gpio_direction_output(enrc2b_lcd_swp_pl, 0);
	enrc2b_stereo_set_orientation(enrc2b_stereo.orientation);

#if !(DC_CTRL_MODE & TEGRA_DC_OUT_ONE_SHOT_MODE)
	tegra_gpio_enable(LCM_TE);
	gpio_request(enrc2b_lcd_swp_pl, "lcd_te");
	gpio_direction_input(LCM_TE);
#endif

#ifdef CONFIG_HAS_EARLYSUSPEND
	enrc2b_panel_early_suspender.suspend = enrc2b_panel_early_suspend;
	enrc2b_panel_early_suspender.resume = enrc2b_panel_late_resume;
	enrc2b_panel_early_suspender.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	register_early_suspend(&enrc2b_panel_early_suspender);
#endif
	...
	err = platform_add_devices(enrc2b_gfx_devices,
				ARRAY_SIZE(enrc2b_gfx_devices));

#if defined(CONFIG_TEGRA_GRHOST) && defined(CONFIG_TEGRA_DC)
	res = nvhost_get_resource_byname(&enrc2b_disp1_device,
	...
	switch (PANEL_MASK(g_panel_id)) {
		...
		case PANEL_ID_SONY_NT_C1:
			...
			enrc2b_dsi.n_osc_off_cmd = ARRAY_SIZE(osc_off_cmd);
			enrc2b_dsi.osc_off_cmd = osc_off_cmd;
			enrc2b_dsi.n_osc_on_cmd = ARRAY_SIZE(osc_on_cmd);
		...
		case PANEL_ID_SHARP_NT_C1:
			...
			enrc2b_dsi_modes[0].h_front_porch = 29;
...
#if defined(CONFIG_TEGRA_GRHOST) && defined(CONFIG_TEGRA_DC)
	if (!err)
		err = nvhost_device_register(&enrc2b_disp1_device);

	res = nvhost_get_resource_byname(&enrc2b_disp2_device,
					 IORESOURCE_MEM, "fbmem");
	if (res) {
	...
	if (!err)
		err = nvhost_device_register(&enrc2b_disp2_device);
#endif
	...
	if( (board_mfg_mode() == BOARD_MFG_MODE_OFFMODE_CHARGING) ) {
		if (board_zchg_mode() & 0x1)	/*off-mode charge with china sku, need to draw battery*/
			enrc2b_disp1_backlight_data.backlight_status = BACKLIGHT_SKIP_WHEN_PROBE;
		else				/*off-mode charge without china sku, do not need to open backlight */
			enrc2b_disp1_backlight_data.backlight_status = BACKLIGHT_DISABLE;
	}

	if (!err)
		err = platform_add_devices(enrc2b_bl_devices,
